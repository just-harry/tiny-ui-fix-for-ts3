...
foreach ($Path in @((Join-Path $Data.TinyUIFixPatchPath AssemblyInfo.cs), (Join-Path $Data.TinyUIFixCoreBridgePath AssemblyInfo.cs)))
{
	sed -b -i $(if ($IsMacOS) {''}) -E -e "s/(\bAssemblyFileVersion\s*\(\s*`")[0-9]+\.[0-9]+(\.[0-9]+)?(\.[0-9]+)?/\1$Version.0/" -e "s/(\AssemblyVersion\s*\(\s*`")[0-9]+\.[0-9]+(\.[0-9]+)?(\.[0-9]+)?/\1$Version.0/" -- $Path
}
